...
# Parse arguments
DEPLOY_MODE="docker"  # Default to docker deployment
while [[ $# -gt 0 ]]; do
    case $1 in
        --local)
            DEPLOY_MODE="local"
        ...
        --help|-h)
            ...
            echo "  --local     Setup for local development (requires .NET SDK and Node.js)"
...
echo -e "Deployment mode: ${BLUE}$DEPLOY_MODE${NC}\n"
...
if [ "$DEPLOY_MODE" = "docker" ]; then
    ...
    print_step "Starting API and Web containers..."
    docker compose up -d api web pgadmin

    # Wait for API to be healthy
    print_step "Waiting for API to be healthy..."
    sleep 10
    RETRIES=30
    until curl -sf http://localhost:5000/health > /dev/null 2>&1 || [ $RETRIES -eq 0 ]; do
        echo -n "."
        sleep 2
        RETRIES=$((RETRIES-1))
    done
    ...
    if [ $RETRIES -eq 0 ]; then
        print_warning "API health check timed out. Check logs with: docker compose logs api"
    else
        print_success "API is healthy"
    fi

    print_header "Setup Complete!"
    echo -e "All services are running in Docker containers."
    echo -e ""
    echo -e "Services:"
    ...
    echo -e "  ${GREEN}./scripts/run.sh stop${NC}    - Stop all services"
    ...
    echo -e ""
    echo -e "  ${GREEN}./scripts/run.sh --local${NC}        - Start all services locally"
    echo -e "  ${GREEN}./scripts/run.sh --local api${NC}    - Start only the API"
    echo -e "  ${GREEN}./scripts/run.sh --local web${NC}    - Start only the web dashboard"
    echo -e ""
    echo -e "Or manually:"
